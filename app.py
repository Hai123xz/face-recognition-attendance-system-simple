...
def _attendance_worker(source, class_name, late_minutes):
    ...
    try:
        ...
        while attendance_status["running"]:
            ...
            if len(faces) == 0:
                ...
                for face in faces:
                    ...
                    if frame_idx % RECOGNIZE_INTERVAL == 0 or key not in last_results:
                        sid, dist, conf = match_embedding(embedding, known_db)
                        last_results[key] = (sid, dist, conf)
                    else:
                        sid, dist, conf = last_results[key]
                ...
                handle_absent_web(conn, session_id, state)
            # HUD
            hud = f"Class: {class_name} | Students: {len(state)}"
        ...
        print(f"[Attendance] CRITICAL ERROR: {str(e)}")
