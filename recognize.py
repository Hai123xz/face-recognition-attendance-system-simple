...
def match_embedding(
    ...
    all_vecs = np.vstack([v for refs in known_db.values() for v in refs])
    all_ids = [sid for sid, refs in known_db.items() for _ in refs]

    # 2. Batch cosine distance
    dists = cdist(query_vec[None, :], all_vecs, 'cosine')[0]
    idx = np.argmin(dists)
    best_dist = dists[idx]
    best_student = all_ids[idx]

    # 3. Threshold check
    if best_dist <= tolerance:
        return best_student, best_dist, distance_to_confidence(best_dist, tolerance)
